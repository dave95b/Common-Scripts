    ...
    public class DropdownPropertyDrawer : PropertyDrawer
    {
        ...
        public override void DrawProperty(SerializedProperty property)
        {
                     ...
                     fieldInfo.FieldType == this.GetElementType(valuesFieldInfo))
            {
                ...
                UpdateValue(selectedValue, property);
            ...
            else if (valuesFieldInfo.GetValue(target) is IEnumerable enumerable)
            {
                // Selected value
                object selectedValue = fieldInfo.GetValue(target);
                UpdateValue(selectedValue, property);
                ...
                foreach (object element in enumerable)
                {
                    index++;
                    values.Add(element);
                    displayOptions.Add(element.ToString());
        ...
        private void UpdateValue(object newValue, SerializedProperty property)
        {
            if (newValue != lastValue)
            {
                ...
                if (mb != null)
                {
                    mb.SendMessage("OnValidate", value: null, SendMessageOptions.DontRequireReceiver);
                    ClearConsole();
                }
